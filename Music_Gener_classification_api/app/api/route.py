...
from app.services.model_loader import cnn_model, aug_model, resnet_model
...
async def predict_genre(file: UploadFile = File(...)):
    ...
    resnet_pred = predict_full_song_overlap(resnet_model, audio_path)

    return {
        "cnn_prediction": {"genre":cnn_pred[0],"probability":cnn_pred[1]},
        "augmented_cnn_prediction": {"genre":aug_pred[0],"probability":aug_pred[1]},
        "resnet18_prediction": {"genre":resnet_pred[0],"probability":resnet_pred[1]}
    }